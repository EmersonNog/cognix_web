      ...
      <div className={containerClass}>
        <div className="mb-12 grid grid-cols-[1.7fr_4fr] gap-10 max-[820px]:grid-cols-1 max-[820px]:gap-8">
          <div className="max-[820px]:max-w-[360px]">
            <Brand href="/" className="mb-[14px] text-white" />
          ...
          <div className="grid grid-cols-4 gap-8 max-[820px]:grid-cols-2 max-[520px]:gap-x-6 max-[520px]:gap-y-8">
            {footerColumns.map((column) => (
              <div key={column.title} className="min-w-0">
                <h5 className="mb-3 font-[var(--font-display)] text-[12px] font-bold uppercase tracking-[0.08em] text-[#F1F5F9]">
                  {column.title}
                </h5>
                {column.links.map((link) => {
                  ...
                  return (
                    <a
                      ...
                      className="block py-[5px] text-[13.5px] leading-[1.35] text-[#94A3B8] transition-colors duration-200 hover:text-white"
                    >
                      {link.label}
                    </a>
                  )
